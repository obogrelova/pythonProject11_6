...
async def send_tips(message:Message):
    tips = [
        ...
        'Совет 2: Контролируйте свои расходы и избегайте импульсивных покупок.',
        'Совет 3: Откладывайте часть доходов на сбережения.',
        'Совет 4: Обучайтесь основам финансовой грамотности.',
        'Совет 5: Используйте кредитные карты с умом и вовремя погашайте задолженность.',
        'Совет 6: Планируйте крупные покупки заранее и старайтесь избегать долгов.'
    ]
    tip = random.choice(tips)
    await message.answer(tip)

@dp.message(F.text == 'Личные финансы')
async def finances(massage: Message, state: FSMContext):
...
async def finances(massage: Message, state: FSMContext):
    ...
    cursor.execute('''UPDATE users SET category1 = ?, expenses1 = ?, category2 = ?, expenses2 = ?, category3 = ?, expenses3 = ? WHERE telegram_id = ?''',
                   (data[category1], data[expenses1], data[category2], data[expenses2], data[category3], data[expenses3], float(message.text), telegram_id))
    conn.commit()
    await state.clear()
